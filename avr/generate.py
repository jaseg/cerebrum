...
def generate(desc, device, devicename, builddate, target = 'all'):
    ...
    subprocess.call(['/usr/bin/env', 'make', '-C', os.path.dirname(__file__), 'clean', target], env=make_env)
...
class TestBuild(unittest.TestCase):
    ...
    def test_basic_build(self):
        generate({'members': {}, 'version': 0.17}, {'chip': 'atmega2560'}, 'test', '2012-11-14 20:11:01')

class TestCommStuff(unittest.TestCase):
    
    def setUp(self):
        generate({'members': {}, 'version': 0.17}, {'chip': 'atmega2560'}, 'test', '2012-11-14 20:11:01', 'test')
        self.terminated = False

    def new_test_process(self):
        #spawn a new communication test process
        p = subprocess.Popen([os.path.join(os.path.dirname(__file__), 'comm-test')], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
